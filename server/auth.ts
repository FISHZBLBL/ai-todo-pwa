...
export async function login(password: string) { if (!(await bcrypt.compare(password, config.PASSWORD_HASH))) return null; return jwt.sign({ sub: 'single-user' }, config.JWT_SECRET, { expiresIn: '30d', issuer: 'ai-todo' }) }
