...
2. 日期与时间严格分离。只有用户明确说出几点时才设置 startTime；“上午/下午/晚上/中午/傍晚”没有具体几点时，startTime 必须为 null，可保留在标题。
3. “待会/等会/有空/之后”等不可靠表达不得猜日期，dueDate 与 dateRange 为 null。
...
7. dueDate 使用 YYYY-MM-DD，startTime/endTime 使用 HH:mm。结束时间只有用户明确给出时设置。
8. “今天/明天”等必须根据请求中提供的 currentLocalDateTime、timezone 和 locale 计算，不使用服务器时间。
9. dueDate 与 dateRange 不得同时存在；没有日期则二者都是 null。
