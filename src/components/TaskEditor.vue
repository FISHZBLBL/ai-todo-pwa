...
function save() {
  ...
  const dueDate = form.dueDate || null, startTime = hasExactDate ? form.startTime || null : null
  emit('save', { title: form.title.trim(), dueDate, startTime, endTime: startTime ? form.endTime || null : null, ...automaticReminder(dueDate, startTime), note: form.note || null, url: form.url || null, pinned: form.pinned, dateRange: form.dueDate ? null : props.task?.dateRange ?? null })
}
...
<template><BottomSheet :title="quick ? '快速添加任务' : '编辑任务'" @close="emit('close')">
  <form class="editor" @submit.prevent="save">
    ...
    <p v-if="form.dueDate && form.startTime" class="time-reminder">将在开始时间提醒</p>
