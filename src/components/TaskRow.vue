...
function onDragStart(event: PointerEvent) {
  ...
  event.currentTarget instanceof HTMLElement && event.currentTarget.setPointerCapture(event.pointerId)
  longPressTimer = window.setTimeout(() => { dragging.value = true; suppressClick.value = true; navigator.vibrate?.(12); emit('reorder-start', props.task.id) }, 450)
}
function onDragMove(event: PointerEvent) {
  ...
  if (dragging.value) { event.preventDefault(); emit('reorder-move', props.task.id, event.clientX, event.clientY) }
}
function onDragEnd(event: PointerEvent) { event.preventDefault(); clearLongPress(); if (dragging.value) { dragging.value = false; emit('reorder-end', props.task.id) } }
...
<template>
  <div class="task-swipe" :class="{ overdue: isOverdue(task), dragging }" :data-task-id="task.id">
    ...
    <article class="task-row" :style="{ transform: `translateX(${delta}px)` }" @click.capture="closeRevealed" @pointerdown="onStart" @pointermove="onMove" @pointerup="onEnd" @pointercancel="onEnd">
      ...
      <button v-if="reorderable && !completedView" class="drag-handle" aria-label="长按并拖动以排序" title="长按拖动排序" @contextmenu.prevent @pointerdown.stop="onDragStart" @pointermove.stop="onDragMove" @pointerup.stop="onDragEnd" @pointercancel.stop="onDragEnd"><GripVertical :size="19" :stroke-width="1.8"/></button>
