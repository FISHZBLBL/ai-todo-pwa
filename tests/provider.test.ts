...
describe('AI Provider 重试策略', () => {
  ...
  it('非法 JSON 安全重试一次，保持原始用户内容且降级关闭 JSON mode', async () => {
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce({ ok: true, json: async () => ({ choices: [{ message: { content: JSON.stringify({ tasks: [{ title:'测器件', dueDate:'2026-08-28', dateRange:null, startTime:'15:00', endTime:null, url:null, reminder:null }] }) } }] }) })
