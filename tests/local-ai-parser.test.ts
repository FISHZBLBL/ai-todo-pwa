...
describe('本地 AI 失败兜底解析', () => {
  ...
  it('继承并列上下文中的日期但不创造具体时间', () => {
    ...
    expect(tasks.map(item => item.date)).toEqual(['2026-08-28', '2026-08-28', '2026-08-28', '2026-08-28'])
    expect(tasks.every(item => item.time === null)).toBe(true)
    expect(tasks[0].title).toBe('下午去实验室')
  })
  it('新的日期范围替换继承上下文', () => { const tasks = parseLocalInbox('明天看论文，周末收拾屋子，买菜', now); expect(tasks[0].date).toBe('2026-08-28'); expect(tasks[1].dateRange).toEqual({ start:'2026-08-29', end:'2026-08-30' }); expect(tasks[2].dateRange).toEqual(tasks[1].dateRange) })
