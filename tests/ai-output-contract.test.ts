...
describe('AI response contract', () => {
  ...
  it('接受模糊提醒', () => expect(parseAIResponse({ tasks: [{ title: '做实验', dueDate: null, reminders: [{ requested: true, date: '2026-08-30', time: null, period: 'evening' }] }] }).tasks[0]?.reminders[0]).toEqual({ requested: true, date: '2026-08-30', time: null, period: 'evening' }))
  ...
  it('周末是日期语义而不是 period，安全清理模型的 weekend 误填', () => expect(parseAIResponse({ tasks: [{ title: '整理资料', reminders: [{ requested: true, date: '2026-09-05', time: null, period: 'weekend' }] }] }).tasks[0]?.reminders[0]).toEqual({ requested: true, date: '2026-09-05', time: null, period: null }))
  ...
  it('旧 singular reminder 只在 normalization 边界转为 reminders', () => expect(parseAIResponse({ tasks: [{ title: '做实验', reminder: { requested: true, date: '2026-08-30', time: '20:00' } }] }).tasks[0]?.reminders).toEqual([{ requested: true, date: '2026-08-30', time: '20:00', period: null }]))
})
